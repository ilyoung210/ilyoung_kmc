...
def create_heatmap_gif_for_seeds(Nx, Ny, segments, seeds, filename, labels):
    ...
    cols = len(seeds)
    ar = Ny / float(Nx)
    wfac = max(1.0, math.sqrt(ar))
    hfac = max(1.0, math.sqrt(1.0 / ar))
    fig, axes = plt.subplots(
        1,
        cols,
        figsize=(cols * 2.5 * wfac, 2.5 * hfac),
    )
...
def run_scatter_mode(
    ...
    def plot_heatmaps(indices, title):
        ...
        fig, axes = plt.subplots(
            rows,
            cols,
            figsize=(cols * 2.5 * wfac, rows * 2.5 * hfac),
        )
    ...
    fig_low = plot_heatmaps(low_idx, 'Low Polarization (bottom 10%)')
    if fig_low:
        fig_low.savefig(os.path.join('polarization', 'low.pdf'), bbox_inches='tight')
        plt.close(fig_low)
    fig_mid = plot_heatmaps(mid_idx, 'Middle 10% Polarization')
    if fig_mid:
        fig_mid.savefig(os.path.join('polarization', 'mid.pdf'), bbox_inches='tight')
        plt.close(fig_mid)
    fig_high = plot_heatmaps(high_idx, 'High Polarization (top 10%)')
    if fig_high:
        fig_high.savefig(os.path.join('polarization', 'high.pdf'), bbox_inches='tight')
        plt.close(fig_high)

    fig_O_low = plot_heatmaps(low_O, 'Low O fraction (bottom 10%)')
    if fig_O_low:
        fig_O_low.savefig(os.path.join('O_phase', 'low.pdf'), bbox_inches='tight')
        plt.close(fig_O_low)
    fig_O_mid = plot_heatmaps(mid_O, 'Middle 10% O fraction')
    if fig_O_mid:
        fig_O_mid.savefig(os.path.join('O_phase', 'mid.pdf'), bbox_inches='tight')
        plt.close(fig_O_mid)
    fig_O_high = plot_heatmaps(high_O, 'High O fraction (top 10%)')
    if fig_O_high:
        fig_O_high.savefig(os.path.join('O_phase', 'high.pdf'), bbox_inches='tight')
        plt.close(fig_O_high)
