...
def _calc_heatmap_figsize(Nx, Ny, long_side=6.0, short_min=2.0):
    """Return (w,h) keeping aspect without becoming extreme."""
    if Nx >= Ny:
        width = long_side
        height = max(short_min, long_side * Ny / float(Nx))
    else:
        height = long_side
        width = max(short_min, long_side * Nx / float(Ny))
    return width, height


def create_heatmap_gif_for_seeds(Nx, Ny, segments, seeds, filename, labels):
    """Create GIF showing heatmaps for given seeds. Layout adapts to aspect."""
    if not seeds:
        return
    ...
    cols = min(2, len(seeds))
    rows = int(math.ceil(len(seeds) / float(cols)))
    w_unit, h_unit = _calc_heatmap_figsize(Nx, Ny, long_side=4.0, short_min=2.0)

    fig, axes = plt.subplots(
        rows,
        cols,
        figsize=(cols * w_unit, rows * h_unit),
    )
...
def run_scatter_mode(
    ...
    def plot_heatmaps(indices, title):
        ...
        figs = []
        ar = Ny / float(Nx)
        orientation_vertical = ar < 1.0
        w_unit, h_unit = _calc_heatmap_figsize(Nx, Ny, long_side=6.0, short_min=2.0)
        per_page = 2
    ...
    if fig_O_low_list:
        with PdfPages(os.path.join('O_phase', 'low.pdf')) as pdf:
            for fg in fig_O_low_list:
                pdf.savefig(fg, bbox_inches='tight')
                plt.close(fg)
    fig_O_mid_list = plot_heatmaps(mid_O, 'Middle O fraction')
    if fig_O_mid_list:
